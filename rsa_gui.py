...
class RSAEncryptionGUI:
    ...
    def generate_keys(self):
        ...
        try:
            ...
            private_key_path = os.path.join(key_dir, f"private_{timestamp}.pem")
            ...
            public_key_path = os.path.join(key_dir, f"public_{timestamp}.pem")
            ...
            # 保存备注文件
            if remark:
                private_remark_file = os.path.join(key_dir, f"private_{timestamp}.txt")
                with open(private_remark_file, "w", encoding="utf-8") as f:
                    f.write(remark)
                
                public_remark_file = os.path.join(key_dir, f"public_{timestamp}.txt")
    ...
    def open_key_management(self):
        ...
        generate_btn = ttk.Button(btn_frame, text="生成密钥对", command=lambda: [self.generate_keys(), self._load_key_list(), self._update_key_status(key_status_frame)], style="Accent.TButton")
        generate_btn.pack(fill=tk.X, padx=2, pady=2)
        
        import_private_btn = ttk.Button(btn_frame, text="导入私钥", command=lambda: [self.import_private_key(), self._load_key_list(), self._update_key_status(key_status_frame)])
        import_private_btn.pack(fill=tk.X, padx=2, pady=2)
        
        import_public_btn = ttk.Button(btn_frame, text="导入公钥", command=lambda: [self.import_public_key(), self._load_key_list(), self._update_key_status(key_status_frame)])
        ...
        # 初始更新密钥状态
        self._update_key_status(key_status_frame)
        ...
        self.current_key_type = None  # "private" 或 "public"
        ...
        self.key_remark_entry.bind("<KeyRelease>", self._save_remark)
    ...
    def _update_key_status(self, key_status_frame):
        """更新密钥状态显示"""
        # 清除所有子组件
    ...
    def _on_key_select(self, event):
        ...
        if selected_index < private_key_count:
            # 选中的是私钥
            self.current_key_type = "private"
            self.current_key_index = selected_index
            self.current_key_info = self.private_keys[selected_index]
            self.key_remark_entry.delete(0, tk.END)
            self.key_remark_entry.insert(0, self.current_key_info["remark"])
        else:
            # 选中的是公钥
            self.current_key_type = "public"
            self.current_key_index = selected_index - private_key_count
            self.current_key_info = self.public_keys[self.current_key_index]
            self.key_remark_entry.delete(0, tk.END)
            self.key_remark_entry.insert(0, self.current_key_info["remark"])
    
    def _save_remark(self, *args):
        """保存密钥备注"""
        if self.current_key_info and self.current_key_type:
            remark = self.key_remark_entry.get().strip()
            # 更新内存中的备注
            self.current_key_info["remark"] = remark
            # 保存到文件
            timestamp = self.current_key_info["timestamp"]
            remark_file = os.path.join("key", f"{self.current_key_type}_{timestamp}.txt")
